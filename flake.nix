    ...
    { self, ... }:
    {
      ...
      perSystem = { pkgs, lib, ... }:
        let
          ...
          deepinPkgs = lib.filterAttrs (_: value: builtins.typeOf value == "set") deepinScope;
          ...
          deepinPkgsDbg = with lib.attrsets; mapAttrs' getDbgVersion deepinPkgs;
        in
        {
      ...
      flake.nixosModules = {
        default = self.flake.nixosModules.dde-nixos;
        dde-nixos = { config, lib, pkgs, utils, ... }:
          with lib;
          let
